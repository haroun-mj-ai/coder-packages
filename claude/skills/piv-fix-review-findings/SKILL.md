...
**Model: sonnet, medium** by default (this is bounded, well-specified execution — a review finding already says
what's wrong). Escalate a single finding to `Agent(model: "opus", effort: "high")` only if it's genuinely
architectural (the reviewer flagged a wrong approach, not a wrong line) — most findings don't need that.
