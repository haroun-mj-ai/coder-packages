...
**Model: sonnet, high effort**, run in the current session (or `Agent(model: "sonnet", effort: "high")` if
delegating) — this is the cheap, same-session pre-commit gate, deliberately lighter than `piv-review-pr`'s
fresh-context adversarial pass later. Don't dispatch this one to opus or a different model family; that
independence is `piv-review-pr`'s job, not this one's, and running two identical-tier passes back to back adds
cost without adding a genuinely different perspective.
