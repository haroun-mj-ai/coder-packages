---
name: piv-slice-epic
description: Slice an epic (with its architecture decisions) into PIV-sized tickets with a dependency graph, then create them in your tracker (Linear via the Linear MCP — this project's tracker — or Jira via the Atlassian MCP, GitHub, local). Accepts the epic and its architecture as one doc or as an epic plus a separate linked architecture page. Turns a large strategic doc into the discrete units of work that the PIV loop consumes.
argument-hint: "[epic + its linked architecture page (a Linear parent issue/project, paths, or Confluence/Jira URLs); a PRD for greenfield]"
---
...
- `$ARGUMENTS` — the **epic** to slice **and its architecture decisions**. These arrive as **one doc or two**:
  ...
  When the input is a Linear parent issue (this project's tracker), fetch it with `mcp__linear-server__get_issue`
  — its architecture may be a linked document (`list_documents`/`get_document` on the issue's project) or an
  `## Architecture` section on the issue itself. When the inputs are Confluence/Jira URLs or keys instead, fetch
  via the Atlassian MCP. **This is the load-bearing input:** the architecture names the seams, data model, and
  missing pieces the slices must respect.
...
Slicing needs enough codebase awareness to judge what's independent vs dependent — file overlap, shared seams. If the session is already oriented, skip this. Otherwise **don't read the bulk of it yourself** — dispatch `Agent(subagent_type: "scout")` (haiku, low) for each named seam/adapter/orchestrator, one message, in parallel: starting from the architecture's named seams, data model, and missing pieces, report what exists, what's reused, and where new code would land. You do the actual independent-vs-dependent judgment from their reports; just enough to slice confidently — not a full re-derivation.
...
Write the tickets to your tracker (Linear via the Linear MCP — `save_issue`, this project's tracker — or Jira via
the Atlassian MCP, GitHub Issues, Archon's tasks) — or to a local `docs/tickets/<epic-slug>.md` if you're solo or
have no tracker. For Linear specifically: `team: "Engineering"`, and set each ticket's **parent** to the epic
issue if one exists, so `piv-plan-implementation`'s parent/sub-issue worktree logic can tell them apart later. Either way, **every ticket carries its own
