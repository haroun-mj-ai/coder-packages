...
**Input intent**: $ARGUMENTS — a PRD, an epic, a brief, or a free-form idea. If it is a Linear parent issue
(`mcp__linear-server__get_issue`, this project's tracker), fetch it first. If it's a Confluence/Jira URL or key
instead, fetch it via the Atlassian MCP first.
...
Only after the calls are made. Pick where it lives. If the intent lives in a **tracker** (a Linear parent issue, a
Confluence epic, a Jira epic), the strong default is a **separate doc linked to the epic, both ways**: the epic
stays pure intent, the architecture (the *how*) lives in its own decision doc beside it, and each links to the
other. Keeping them as two clean, linked sources is what lets `piv-slice-epic` and `piv-plan-implementation` read
intent and architecture separately later. The options:

- **A separate linked doc in your tracker** (recommended when the epic lives in Linear/Confluence/Jira): for
  Linear, `mcp__linear-server__save_document` on the epic's project, linked from the epic issue; for
  Confluence/Jira, create a new page in the epic's space, as a child of the epic, and link it both ways (via the
  Atlassian MCP).
...
- **Slice it into tickets** — feed the doc to `/piv-slice-epic` to break the epic into PIV-sized tickets, and create
  the Linear issues (this project's tracker) — or GitHub issues / Jira tickets — from them.
