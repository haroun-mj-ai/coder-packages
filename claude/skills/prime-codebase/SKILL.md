---
name: prime-codebase
description: Primes the agent with deep codebase understanding by analyzing structure, documentation, and key files. Use when starting work on a codebase, at the beginning of a session, or when you need a fast orientation before planning or implementing. Optionally pulls external task context from Linear issues (this project's tracker) or Jira issues/Confluence pages first.
argument-hint: "[linear-issue-ids] [jira-issue-keys] [confluence-page-ids]"
---
...
**Run this step BEFORE the codebase analysis.** It accepts optional arguments: `[linear-issue-ids] [jira-issue-keys] [confluence-page-ids]`.

- Linear issue ids are this project's tracker (e.g. `ENG-123`), single or comma-separated.
- Jira keys may be a single key (`ACC-2`) or comma-separated (`ACC-2,ACC-3`).
- Confluence page ids are numeric page ids.

**If Linear issue ids are provided:**

1. Call `mcp__linear-server__get_issue` for each id.
2. Call `mcp__linear-server__list_comments` for each — requirements are often only in the comments.
3. Treat the returned description, acceptance criteria, and comments as the task context for everything that follows.
...
- Linear issue(s): id, title, one-line goal, acceptance criteria
